...
| Module             | Purpose                                                                                                                                          |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **app/**           | Bootstrap, paths, runtime - wires config, storage, AI, browser, pipeline hooks; plus form mapping and apply automation (merged from application) |
...
### 9. Combine app and application (Medium Impact)

**Current state**:

- `app/` — bootstrap, paths, runtime (wiring)
- `application/` — form_mapper, service (apply stage automation)

**Recommendation**: Merge `application/` into `app/`. Move `form_mapper.py` and `service.py` into `app/`. Update `app/__init__.py` to export `ApplicationService`, `FormMapper`, `FieldMapping`. All imports `roller.application.`* become `roller.app.`*. Delete `application/` package. Move `tests/unit/application/` to `tests/unit/app/` (or merge into existing app tests).

**Resulting structure**:

```
app/
  __init__.py   # build_runtime, RollerRuntime, BootstrapOverrides, ApplicationService, FormMapper, FieldMapping
  bootstrap.py
  paths.py
  runtime.py
  form_mapper.py
...
**Files to update**: `pipeline/stages.py`, `cli/commands/run.py`, `cursor_sdk/client.py`, `cursor_sdk/sdk.py`, `cli/cursor_agent_helpers.py`, `cli/commands/status.py`, `pipeline/refactor/executor.py`, tests.
...
2. **Phase 1b — Combine app + application** (#9): Merge application into app; update imports and tests.
3. **Phase 2 — Extract pipeline.refactor** (#3): Move to `roller.refactor`. Update imports and CLI references.
4. **Phase 3 — tools subpackages** (optional): Add `tools/role_packets/` etc. if tool count justifies it (#5).
5. **Phase 4 — core/shared migration**: Complete the migration (#1). Highest impact; requires careful import updates and test runs.
