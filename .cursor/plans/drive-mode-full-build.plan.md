...
todos:
  ...
  - id: glossary-expander
    content: Add glossaryExpander.ts with shortcut phrases; wire after filler cleaning
    status: pending
  - id: sanitization
    content: Add context sanitization (truncation, injection stripping); cursorDrive.maxPromptLength
    status: pending
  - id: approval-gates
    content: Add approvalGates.ts: scan prompt/response for dangerous ops; showWarningMessage gate
    status: pending
  - id: mcp-deeplink
    content: Implement MCP auto-registration via Cursor deep link (registerMcpViaDeepLink)
    status: pending
  - id: audio-feedback
    content: Create audioFeedback.ts with WebView chime player (playChime 1=ON, 2=OFF)
    status: pending
  - id: wake-ack
    content: Add wake-word acknowledgment in pipeline (speak + status bar + empty-text handling)
    status: pending
  - id: dev-preset
