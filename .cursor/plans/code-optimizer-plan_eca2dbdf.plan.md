...
overview: Create a specialized `code-optimizer` subagent with supporting commands, skills, and hooks. Ensure it functions nicely alongside the broader Cursor Plugin ecosystem. Systematically refactor identified memory and performance inefficiencies across the codebase.
todos:
  - id: create-agent
...
We will build a `code-optimizer` subagent ecosystem that continuously monitors and suggests improvements. Then, we will address a series of specific inefficiencies found in the current extension code. We will ensure this is structured following best practices for Cursor Plugins (like Cursor Team Kit).

## 1. Subagent Ecosystem (Plugin-Aware)

- **Subagent (`.cursor/agents/code-optimizer.md`)**: A specialized agent that looks for duplicated logic, un-cached config reads, O(n) lookups in hot paths, and unbounded memory growth. Designed to be reusable across workspaces.
...
- **Command (`.cursor/commands/optimize.md`)**: A slash command (`/optimize`) to explicitly invoke the optimizer on the current file or specific module.
...
- **Model Selection Duplication**: `vscode.lm.selectChatModels` and the "cheap model" fallback logic is duplicated across `responseFormatter.ts`, `commsAgent.ts`, and `modelSelector.ts`. We will extract this into a shared utility (`modelUtils.ts`).
...
- **ShareScreen HTML Memory**: The ~360-line inline HTML string in `shareScreen.ts` increases memory per WebviewPanel instance. We will extract this into a static template file or string module.