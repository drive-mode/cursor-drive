...
2. **`contributes.views`** — Add a WebviewView inside that container:
   ```json
   "views": {
     "cursorDrive": [{
...
3. **`WebviewViewProvider`** — Register a provider that builds HTML (chat-like UI, operator list, Agent Screen summary, etc.). Full control over layout, styling, and behavior.

4. **Coexist with chat** — User keeps Cursor's chat for native Agent/Plan/Ask; Drive panel is a dedicated surface for Drive-specific features (voice status, operators, Agent Screen, etc.).
