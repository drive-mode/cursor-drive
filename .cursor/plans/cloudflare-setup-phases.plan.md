---
planId: cloudflare-setup-phases
planType: task
...
todos:
  - id: phase1-token-fix
    content: Fix token logging in create-cloudflare-token.mjs (clipboard or instructions only; audit scripts for console.log secrets)
    status: pending
  - id: phase1-disclaimer
    content: Add deployment opt-in disclaimer to cloudflare-workers-mcp-cicd.md
    status: pending
  - id: phase1-user-paths
    content: Replace user-specific paths in mcp-user-setup.md with platform-agnostic placeholders
    status: pending
  - id: phase2-secrets
    content: Document GitHub secrets and local .env (CLOUDFLARE_API_TOKEN, CLOUDFLARE_ACCOUNT_ID)
    status: pending
  - id: phase2-verify
    content: Run cloudflare:verify and Cloudflare Token Test workflow
    status: pending
  - id: phase3-worker
    content: (Optional) Scaffold Worker project, wrangler config, deploy workflow, user MCP config, docs
    status: pending
  - id: phase4-terraform
    content: (Optional) Terraform IaC for Worker + KV/D1/DNS
    status: pending
isProject: false
---
