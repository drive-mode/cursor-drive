...
planId: cursor-primitives-bootstrap
planType: task
...
todos:
  - id: reroot-git
    content: Re-root git to `c:\Users\harri\Documents\Coding Projects\fun\roller` (new repo or otherwise ensure this folder is the git root). Ensure no secrets/home-directory files are tracked.
    status: pending
...
- **Shell subagents**: do git-root/reroot work safely and verify repo boundaries.
...
Your initial status shows git rooted at `C:/Users/harri` (home directory). Before adding more automation (hooks, CLI, background agents), we should re-root git to `...\fun\roller` so:
