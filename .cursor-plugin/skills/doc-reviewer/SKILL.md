...
- User asks to "review", "check", or "audit" the docs (no slash command)
- User mentions "stale", "outdated", or "wrong docs"

**Relationship:** **doc-review** is the slash-command version (explicit `/doc-review`, checklist only). Use **doc-reviewer** when the agent should apply the full protocol (file inventory, severity classification, anti-patterns) or when the user asks to review/audit docs without a slash command.
