...
| Prompt optimizer                             | mvp-gaps: prompt-optimizer                | promptOptimizer.ts reads config, calls routing model, QuickPick diff, returns approved prompt | Pending               |
| Wake/submit words                            | mvp-gaps: wake-submit-words               | Wake word strips and activates; submit word strips and submits without optimization           | Pending               |
...
| Tangent keyword wiring                       | mvp-gaps: tangent-wiring                  | "tangent [task]" at input start → agentRegistry.spawn(); status bar update                    | Pending               |
| **prd-safety-config**                        |                                           |                                                                                               |                       |
...
| Mode switching confirmation                  | mvp-gaps: mode-confirm                    | requireConfirmation=true → QuickPick before mode switch                                       | Pending               |
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------ | --------- |
| mvp-gaps          | prompt-optimizer, wake-submit-words, tangent-wiring, mode-confirm                                                                    | Pending   |
